...
export LAG="$(/opt/kafka/bin/kafka-consumer-groups.sh --bootstrap-server "$BOOTSTRAPSERVER" --describe --group "$GROUP"| awk 'BEGIN{maxLag=   0}{if ($6>0+maxLag) maxLag=$6} END{print maxLag}')"
...
do
LAG="$(/opt/kafka/bin/kafka-consumer-groups.sh --bootstrap-server "$BOOTSTRAPSERVER" --describe --group "$GROUP"| awk 'BEGIN{maxLag=   0}{if ($6>0+maxLag) maxLag=$6} END{print maxLag}')"
...
do
    ...
    export MESSAGECOUNT="$(/opt/kafka/bin/kafka-run-class.sh kafka.tools.GetOffsetShell --broker-list "$BOOTSTRAPSERVER" --topic "$TOPIC" --time -1 --offsets 1 | awk -F  ":" '{sum += $3} END {print sum}')"
